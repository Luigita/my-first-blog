        ...
        if event == Sg.WINDOW_CLOSED or event == "Termina":
            ...
            while a != len(df.index):
                ...
                if encodedImage == encodedImage:
                    ...
                    df.loc[a, "blob"] = f"=COLLEG.IPERTESTUALE(\"{unzippedFolder}/Foto persiane/{df['riferimento'].values[a]}{a}.jpg\"; \"{df['riferimento'].values[a]}{a}.jpg\")"
            ...
            while a != len(df.index):
                ...
                if encodedDisegno == encodedDisegno:
                    ...
                    df.loc[a, "disegno"] = f"=COLLEG.IPERTESTUALE(\"{unzippedFolder}/Disegno persiane/{df['riferimento'].values[a]}{a}.png\"; \"{df['riferimento'].values[a]}{a}.png\")"
            ...
            df_merged1.to_csv(zip_dir + "/" + zip_name + "/Persiane.csv", index=False, sep=";")
            ...
            while a != len(df.index):
                ...
                if encodedImage == encodedImage:
                    ...
                    df.loc[a, "blob"] = f"=COLLEG.IPERTESTUALE(\"{unzippedFolder}/Foto tapparelle/{df['riferimento'].values[a]}{a}.jpg\"; \"{df['riferimento'].values[a]}{a}.jpg\")"
                    imgFile = open(imageName, "wb")
                    imgFile.write(decodedImage)
                    imgFile.close()
                a = a + 1
            ...
            while a != len(df.index):
                ...
                if encodedDisegno == encodedDisegno:
                    ...
                    df.loc[a, "blob"] = f"=COLLEG.IPERTESTUALE(\"{unzippedFolder}/Disegno tapparelle/{df['riferimento'].values[a]}{a}.png\"; \"{df['riferimento'].values[a]}{a}.png\")"
                    imgFile = open(disegnoName, "wb")
                    imgFile.write(decodedDisegno)
                    imgFile.close()
                a = a + 1
            ...
            df_merged2.to_csv(zip_dir + "/" + zip_name + "/Tapparelle.csv", index=False, sep=";")

            returned_value = Sg.popup_ok("Terminato", keep_on_top=True)
